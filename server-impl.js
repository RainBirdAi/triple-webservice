...
function processQuery(apiKey, subject, relationship, data, req, res) {
  router.querySubjectRelationship(
    apiKey,
    subject,
    relationship,
    data,
...
app.get("/static/namequery/:subject/:relationship", function (req, res) {
  rdf.processLabelQuery(req.params.subject, req.params.relationship, function (
    err,
    data
  ) {
    if (err) {
      ...
      console.log(
        "res-success - " + req.params.subject + " - " + req.params.relationship
      );
      senddata(res, data);
    }
  });
...
app.get("/:key/securequery/:subject/:relationship", function (req, res) {
  ...
  if (typeof authorization == "undefined") {
    res
      .status(401)
      .send({ status: "ERROR", message: "Missing Authorization header!" });
  } else {
    var data = {
      query: req.query,
      body: {},
